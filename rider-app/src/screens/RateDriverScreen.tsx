...
import { submitRating } from '../api/rideApi';
import type { NativeStackScreenProps } from '@react-navigation/native-stack';
import type { RootStackParamList } from '../navigation/RootNavigator';

type Props = NativeStackScreenProps<RootStackParamList, 'RateDriver'>;

export default function RateDriverScreen({ navigation, route }: Props) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.content}>
        <Text style={typography.h1}>{t('rating.howWasYourTrip')}</Text>
        <Text style={[typography.body, { color: colors.gray600, marginTop: 4, marginBottom: spacing.xl }]}>
          {t('rating.rateDriverName', { driverName })}
        </Text>
