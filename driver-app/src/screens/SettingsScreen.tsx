  ...
  useEffect(() => {
    ...
    getDoc(doc(db, 'drivers', uid))
      .then((snap) => {
        ...
        if (data?.vehicleModel) setVehicle(data.vehicleModel);
    ...
    <View style={styles.container}>
      ...
      <View style={styles.profileCard}>
        <View style={styles.avatar}>
          <Text style={styles.avatarText}>{(name || 'D').charAt(0).toUpperCase()}</Text>
        </View>
        <View style={{ flex: 1 }}>
          <Text style={typography.h3}>{name || t('settings.driver')}</Text>
      ...
      <View style={styles.group}>
        ...
        <Pressable style={styles.row} onPress={() => navigation.navigate('Wallet')}>
          ...
          <Text style={[typography.body, { flex: 1 }]}>{t('settings.wallet')}</Text>
          <Ionicons name="chevron-forward" size={18} color={colors.gray400} />
        </Pressable>
      </View>
